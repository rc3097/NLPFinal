...
public class WordVectorIterator implements DataSetIterator {
	...
	public WordVectorIterator(HashMap<String,TweetSet> tweetSets) {
		...
		index = 0;
		miniBatchSize = 5; 
		this.tweetSets = new ArrayList<TweetSet>(tweetSets.values());
	}
	...
	public DataSet next(int num) {
		if (index+num > tweetSets.size())
			num = tweetSets.size() - index;
		...
		return new DataSet(Nd4j.create(features, new int[] { num, 25, minSize }),
				Nd4j.create(labels, new int[] { num, 1, minSize }));
	}
