	...
	public static class ObjectiveFunction<F, L> implements
		...
		private Pair<Double, double[]> calculate(double[] x) {
			...
			for (int i = 0; i < (data.length); i++) {
				prob[i] = getLogProbabilities(data[i], x, encoding,
						indexLinearizer);
			...
			for (int i = 0; i < data.length; i++) {
				for (int j = 0; j < data[i].featureIndexes.length; j++) {
					...
					derivatives[indexLinearizer.getLinearIndex(
							data[i].getFeatureIndex(j), clabel)] += data[i]
							.getFeatureCount(j);
					for (int j2 = 0; j2 < indexLinearizer.numLabels; j2++) {
						int f = indexLinearizer.getLinearIndex(
								data[i].getFeatureIndex(j), j2);
						derivatives[f] -= (Math.exp(prob[i][j2]) * data[i]
								.getFeatureCount(j));
					}
				}
			}
	...
	private static <F, L> double[] getLogProbabilities(EncodedDatum datum,
		...
		for (int i = 0; i < indexLinearizer.numLabels; i++) {
			...
			for (int j = 0; j < datum.featureIndexes.length; j++) {
				logProbabilities[i] += weights[indexLinearizer.getLinearIndex(
						datum.getFeatureIndex(j), i)]
